...
public class AdminServlet extends HttpServlet {
	...
	private UserServiceAPI studentService;
...
/*	private String manageDays(HttpServletRequest request,
	...
	private String manageOrders(HttpServletRequest request,
		...
		try {
			orders1 = adminService.getTodaysOrdersByStatus(PizzaOrder.PREPARING);
			System.out.println("seeing " + orders1.size() + " preparing orders");
			orders2 = adminService.getTodaysOrdersByStatus(PizzaOrder.BAKED);
	...
	private void removeSize(String sizeId) throws ServiceException {
		// convert id to name for removeSize
		int id = Integer.parseInt(sizeId);
		Set<PizzaSize> allSizes = studentService.getPizzaSizes();
		PizzaSize chosenSize = PresentationUtils.getSizeFromSizes(allSizes, id);
		adminService.removePizzaSize(chosenSize.getSizeName());
	}

	private void removeTopping(String toppingId) throws ServiceException {
		// convert id to name for removeTopping
		int id = Integer.parseInt(toppingId);
		Set<Topping> allToppings = studentService.getToppings();
		Topping chosenTopping = PresentationUtils.getToppingFromToppings(
				allToppings, id);
